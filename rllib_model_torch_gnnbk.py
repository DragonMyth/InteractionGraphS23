...
from torch_geometric.nn import GCNConv,GATConv,EdgeConv,DynamicEdgeConv,GATv2Conv
...
class GAT(nn.Module):
    ...
    def __init__(self, channel_in, channel_out, out_dim, num_nodes_per_batch,edge_dim,gcn_layers,fc_layers):
        ...
        for l in gcn_layers:
            ...
            layer = GATv2Conv(prev_channel_size,channel_out_size,add_self_loops=False,edge_dim=edge_dim)
            
            gcn_nn_layers.append((layer,"x, edge_index, edge_attr,return_attention_weights -> x, attention"))
            prev_channel_size = channel_out_size

        self._gcn_model = GeoSequential('x, edge_index, edge_attr,return_attention_weights',gcn_nn_layers)
    ...
    def forward(self, x, edge_index, edge_attr):
        ...
        gcn_out,attention = self._gcn_model(x,edge_index,edge_attr,return_attention_weights=True)
...
class FC_Bypass(nn.Module):
    ''' 
    A network with fully connected layers, where we feed the input z into every layer
    so that the network is more sensitive to the input z.
    '''
    def __init__(self, size_in_x, size_in_z, size_out, layers, append_log_std=False,
                 log_std_type='constant', sample_std=1.0):
        super().__init__()
        self.layers = layers
        self.append_log_std = append_log_std
        
        nn_layers = []
        prev_layer_size = size_in_x
        for l in layers:
            layer_type = l['type']
            if layer_type == 'fc':
                assert isinstance(l['hidden_size'] , int) or l['hidden_size'] =='output'
                hidden_size = l['hidden_size'] if l['hidden_size'] != 'output' else size_out
                layer = SlimFC(
                    in_size=prev_layer_size+size_in_z,
                    out_size=hidden_size,
                    initializer=get_initializer(l['init_weight']),
                    activation_fn=get_activation_fn(l['activation'])
                )
        ...
        self._model = nn.ModuleList(nn_layers)
    ...
    def forward(self, x, z):
        ...
        for i, l in enumerate(self.layers):
            ...
            layer_type = l['type']
            if layer_type == 'fc':
                y_cur = self._model[i](torch.cat([y_cur, z], dim=-1))
            else:
                y_cur = model(y_cur)
        
        if self.append_log_std:
            y_cur = self._model[-1](y_cur)
...
class FC_Phase(nn.Module):
    ...
    def __init__(self, size_in, size_out, hiddens, activations, 
                 ...
                 log_std_type='constant', sample_std=1.0,
                 num_discrete_phase=4):
        super().__init__()
        
        assert num_discrete_phase > 0
        self._discrete_phase = \
            np.linspace(0.0, 2.0*np.pi, num_discrete_phase, endpoint=False)
        
        models = []
        for _ in range(num_discrete_phase):
            layers = []
            prev_layer_size = size_in
        ...
        self._log_std = None
        if append_log_std:
            self._log_std = AppendLogStd(
                type=log_std_type, 
                init_val=np.log(sample_std), 
                dim=size_out)

    def forward(self, x):
        ''' Assume that the last two elements are phase variable (x, y) '''
        ## TODO: should be written in a batch manner
        num_discrete_phase = len(self._discrete_phase)
        theta = torch.atan2(x[...,-1], x[...,-2])
        
        if theta < 0.0:
            theta += 2 * np.pi
        for i in range(num_discrete_phase):
            i_next = (i+1)%num_discrete_phase
            model1 = self._models[i]
            model2 = self._models[i_next]
            phase1 = self._discrete_phase[i]
            phase2 = self._discrete_phase[i_next]
            if theta <= phase2: break

        if i_next == 0:
            phase2 = 2 * np.pi

        alpha = (theta - phase1) / (phase2 - phase1)
        # print(theta, phase1, phase2, alpha)
        y = (1.0-alpha) * model1(x) + alpha * model2(x)
        if self._log_std:
            return self._log_std(y)
        else:
            return y
...
class MOEBaseEval(nn.Module):
    ''' 
    A base policy with Mixture-of-Experts structure
    '''
    DEFAULT_CONFIG = {
        "expert_size_in": None,
        "expert_size_out": None,
        "expert_hiddens": [
            [64, 64],
            [64, 64],
            [64, 64],
    ...
    def __init__(self, config):
        ...
        expert_size_in        = self.config.get("expert_size_in")
        expert_size_out       = self.config.get("expert_size_out")
        expert_hiddens        = self.config.get("expert_hiddens")
        expert_activations    = self.config.get("expert_activations")
        expert_init_weights   = self.config.get("expert_init_weights")
        expert_log_std_types  = self.config.get("expert_log_std_types")
        expert_sample_stds    = self.config.get("expert_sample_stds")
        expert_checkpoints    = self.config.get("expert_checkpoints")
        expert_learnable      = self.config.get("expert_learnable")
        ...
        for i in range(num_experts):
            expert = FC(
                size_in=expert_size_in, 
                size_out=expert_size_out, 
                hiddens=expert_hiddens[i], 
                activations=expert_activations[i], 
                init_weights=expert_init_weights[i],
            )
            if expert_checkpoints[i]:
                if project_dir:
                    checkpoint = os.path.join(project_dir, expert_checkpoints[i])
                else:
                    checkpoint = expert_checkpoints[i]
                expert.load_weights(checkpoint)
            for name, param in expert.named_parameters():
                param.requires_grad = expert_learnable[i]
            experts.append(expert)
        self._experts = nn.ModuleList(experts)
    ...
    def get_cur_weights(self):
        return self._cur_weight

    def num_experts(self):
        return len(self._experts)

class MOEAddEval(MOEBaseEval):
    def __init__(self, config):
        super().__init__(config)

    def forward(self, input_dict):
        x_weight = torch.Tensor(input_dict["x_weight"])
        x_expert = torch.Tensor(input_dict["x_expert"])

        w = softmax_normalized(x_weight, dim=1).unsqueeze(-1)
        x = torch.stack([expert(x_expert) for expert in self._experts], dim=1)
        y = torch.sum(w*x, dim=1)

        self._cur_weight = w
        
        return y

class MOEMulEval(MOEBaseEval):
    def __init__(self, config):
        super().__init__(config)
        self.expert_std = \
            torch.Tensor(self.config.get("expert_sample_stds"))
        assert len(self.expert_std) == self.num_experts()
    ...
    def forward(self, input_dict):
        ...
        expert_std = torch.stack([
            self.expert_std for i in range(w.shape[0])
            ]).unsqueeze(-1)

        z = w / expert_std

        std = 1.0 / torch.sum(z, dim=1)
        mean = std * torch.sum(z * x, dim=1)
...
ModelCatalog.register_custom_model("lstm", LSTMPolicy)
