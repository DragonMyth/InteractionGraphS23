...
from envs import env_humanoid_follower as my_env
...
class HumanoidFollower(gym.Env):
...
class EnvRenderer(er.EnvRenderer):
    def __init__(self, trainers, config, **kwargs):
        ...
        self.cam_params = deque(maxlen=15)
        self.cam_param_offset = None
        if self.env.base_env._goal_type=="maze":
            self.options["ground_size"] = [24.0, 24.0]
            self.options["ground_circle_cut"] = False
