...
class Env(object):
    ...
    def step(self, action_dict_list):
        ''' 
        ...
        for _ in range(self._num_substep):
            for i in range(self._num_agent):
                self._agent[i].actuate(action_dict_list[i])
                    # pose=action_dict_list[i].get('pose'),
                    # vel=action_dict_list[i].get('pose'),
                    # torque=action_dict_list[i].get('pose'),
                    # base_residual_linear_force=action_dict_list[i].get('pose'),
                    # base_residual_angular_force=action_dict_list[i].get('pose'),
                    # base_residual_linear_force_frame=action_dict_list[i].get('pose'),
                    # base_residual_angular_force_frame=action_dict_list[i].get('pose'))

            # for i in range(len(target_poses)):
            #     pose = target_poses[i]
            #     if base_residual_linear_forces:
            #         base_residual_linear_force = base_residual_linear_forces[i]
            #     else:
            #         base_residual_linear_force = None
            #     if base_residual_angular_forces:
            #         base_residual_angular_force = base_residual_angular_forces[i]
            #     else:
            ...
            #         "base_residual_linear_force_frame": base_residual_linear_force_frame,
            #         "base_residual_angular_force_frame": base_residual_angular_force_frame,
            #     }
            #     args_dict["pose"] = pose
            #     self._agent[i].actuate(**args_dict)
            if profile:
...
if __name__ == '__main__':
    ...
    class EnvRenderer(er.EnvRenderer):
        ...
        def use_default_ground(self):
            return self.env._use_default_ground
    ...
    config = {
        ...
        'character':{
            'name': ['agent1'],
            'char_info_module': ['char_info/amass_char_info.py'],
            'sim_char_file': ['data/character/amass.urdf'],
            'ref_motion_scale': [1.0],
            'physics': [{'self_collision': True, 'actuation': 'none'}],
        },
        "ground":{}
    }
