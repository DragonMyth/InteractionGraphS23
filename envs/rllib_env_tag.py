...
from import env_humanoid_tag as my_env
...
class HumanoidTag1(MultiAgentEnv):
...
>> When LSTM is used, the response time is longer, so timeout occurs
>> small value for "rollout_fragment_length" seems work, but might not be good solution?...

When the state dim is large, some small encoder for LSTM might be necessary
'''
