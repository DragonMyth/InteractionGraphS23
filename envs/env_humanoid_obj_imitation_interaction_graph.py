...
class Env(env_humanoid_base.Env):
    def __init__(self, config):
        ...
        self._interaction_kernel = self._config['interaction'].get("interaction_weight_kernel",1)
        ...
        if self._interaction_joints is not None:
            ...
            [joints.append(joint_idx_list[i]) for i in self._interaction_joints]
        ...
        if self._num_agent==2 and self._target_constraints:
            ...
            for i, target_constraint in enumerate(self._target_constraints):
                ...
                if curr_time >= start_time + duration and (self._constraints[i] is not None):
                    self._pb_client.removeConstraint(self._constraints[i])
                    self._constraints[i] = None
            # sim_agent1 = self._sim_agent[0]
            # sim_agent2 = self._sim_agent[1]
            # left_hand_idx = sim_agent1._char_info.joint_idx['LeftHand']
            # right_hand_idx = sim_agent1._char_info.joint_idx['RightHand']
            # left_leg_idx = sim_agent1._char_info.joint_idx['LeftLeg']
            # right_leg_idx = sim_agent1._char_info.joint_idx['RightLeg']

            # agent1_pos,_,_,_ = sim_agent1.get_link_states((left_hand_idx,right_hand_idx,left_leg_idx,right_leg_idx))
            # agent2_pos,_,_,_ = sim_agent2.get_link_states((left_hand_idx,right_hand_idx,left_leg_idx,right_leg_idx))

            # agent1_leg = agent1_pos[2:]
            # agent2_hand = agent2_pos[:2]
            # hand_leg_dist = np.linalg.norm(agent1_leg-agent2_hand,axis=1)
            
            # if hand_leg_dist[0] <= constraint_threshold and (self._constraints[1]['left'] is None): 
            #     constraint = self._pb_client.createConstraint(sim_agent1._body_id,left_leg_idx,sim_agent2._body_id,left_hand_idx,self._pb_client.JOINT_FIXED,np.zeros(3),np.zeros(3),np.zeros(3))
            #     self._constraints[1]['left'] = constraint
            #     print("Attach left hand+leg")

            # if hand_leg_dist[1] <= constraint_threshold and (self._constraints[1]['right'] is None): 
            #     constraint = self._pb_client.createConstraint(sim_agent1._body_id,right_leg_idx,sim_agent2._body_id,right_hand_idx,self._pb_client.JOINT_FIXED,np.zeros(3),np.zeros(3),np.zeros(3))
            #     self._constraints[1]['right'] = constraint
            #     print("Attach right hand+leg")        
    ...
    def print_log_in_step(self):
        if self._verbose and self._end_of_episode:
            ...
            print('=====================================')
    def compute_reference_interaction_mesh(self,agent_type, idx):
        ...
        if self._load_ref_interaction:
            ...
            edge_indices = interaction.build_interaction_graph(self._interaction_type)
    ...
    def get_all_interaction_points(self,agent_type,idx,*args):
        ...
        if "self" in self._interaction_choices:
            ...
            joint_state = np.array(agent.get_joint_cartesian_state_fast(np.array(joint_candidate)))
            interaction_points.append(joint_state)
        if "link" in self._interaction_choices:
        ...
        if "other_agent" in self._interaction_choices:
            ...
            joint_state = np.array(other_agent.get_joint_cartesian_state_fast(np.array(joint_candidate)))
            interaction_points.append(joint_state)
        return np.concatenate(interaction_points,axis=0)
    def compute_distance_weights(self,points,edge_index=None):
    ...
    def reward_data(self, idx):
        ...
        data['kin_interaction_dist_weight'] = self.compute_distance_weights(self._kin_interaction_points[idx],edge_indices)
    ...
    def render(self, rm):
        ...
        if rm.flag['custom3'] and self.current_interaction:
            ...
            for i in range(self._num_agent):
                ...
                all_weight = self.compute_distance_weights(self._kin_interaction_points[i],edge_index)
        ...
        if rm.flag['custom5']:
            ...
            sim_agent1_points,_,_,_ = sim_agent1.get_link_states((left_hand_idx,right_hand_idx,left_leg_idx,right_leg_idx))
            sim_agent2_points,_,_,_ = sim_agent2.get_link_states((left_hand_idx,right_hand_idx,left_leg_idx,right_leg_idx))

            for i,p in enumerate(sim_agent1_points):
                rm.gl.glPushMatrix()
                rm.gl.glTranslatef(p[0],p[1],p[2])
                rm.gl.glScalef(0.14,0.14,0.14)
                rm.gl_render.render_sphere(
                    constants.EYE_T, 0.4, color=COLORS_FOR_AGENTS[0], slice1=16, slice2=16)
                rm.gl.glPopMatrix()

            for i,p in enumerate(sim_agent2_points):
                rm.gl.glPushMatrix()
                rm.gl.glTranslatef(p[0],p[1],p[2])
                rm.gl.glScalef(0.14,0.14,0.14)
                rm.gl_render.render_sphere(
                    constants.EYE_T, 0.4, color=COLORS_FOR_AGENTS[1], slice1=16, slice2=16)
                rm.gl.glPopMatrix()
