...
from fairmotion.ops import motion 
import motion_utils
...
class Env(env_humanoid_base.Env):
    ...
    def create(self):
        ...
        motion_transforms = self._config['character'].get("transform",None)
        ...
        for i in range(self._num_agent):
            ref_motion_all, ref_motion_file_names,_ = \
                motion_utils.load_motions(
                    ...
                    self._verbose)
            if motion_transforms and len(motion_transforms) == self._num_agent:
                for m in ref_motion_all:
                    q,E = np.array(motion_transforms[i][:3]),np.array(motion_transforms[i][3:])
                    T = conversions.Ep2T(E,q,degrees=True)
                    motion.transform(m,T)
    ...
    def callback_reset_prev(self, info):
        ...
        if 'start_time' in info.keys():
            ...
            time = np.random.uniform(0.0, self._ref_motion[0].length())
            print(self._ref_motion[0].length(),self._ref_motion[1].length(),time)
            for i in range(self._num_agent):
    ...
    def get_state_by_key(self, idx, key):
        ...
        if key=='body':
            ...
            state.append(self.state_vr_sensors(idx))
        elif key=='oppo_ee':
            state.append(self.state_oppo_ee(idx))
    ...
    def state_imitation(
        ...
        return np.hstack(state)
    def state_oppo_ee(self,idx):
        oppo_idx = 1-idx
        sim_agent = self._sim_agent[idx]
        rwristIdx = sim_agent._char_info.rwrist

        oppo_sim_agent = self._sim_agent[oppo_idx]

        R_sim, p_sim = conversions.T2Rp(
            sim_agent.get_facing_transform(self.get_ground_height(idx)))
        R_sim_inv = R_sim.transpose()

        '''Observe the position of the opponent ee relative to player ee position'''
        state = []
        '''Include the sim difference vector between oppo and player right wrist'''
        rwrist_sim_p,rwrist_sim_Q,rwrist_sim_v,rwrist_sim_w = sim_agent.get_link_states([rwristIdx])
        rwrist_sim_R = conversions.Q2R(rwrist_sim_Q[0])
        rwrist_sim_R_inv = rwrist_sim_R.transpose()
        rwrist_oppo_sim_p,rwrist_oppo_sim_Q,rwrist_oppo_sim_v,rwrist_oppo_sim_w = oppo_sim_agent.get_link_states([rwristIdx])
        
        ## Sim Oppo rwrist pos in sim facing frame, pos in sim rwrist frame, vel in sim rwrist frame
        state.append(np.dot(R_sim_inv,rwrist_oppo_sim_p[0]-p_sim))
        state.append(np.dot(rwrist_sim_R_inv,rwrist_oppo_sim_p[0]-rwrist_sim_p[0]))
        state.append(np.dot(rwrist_sim_R_inv,rwrist_oppo_sim_v[0]-rwrist_sim_v[0]))
    ...
    def reward_data(self, idx):
        ...
        opponent_idx = 1-idx
        data['oppo_sim_root_pQvw'] = self._sim_agent[opponent_idx].get_root_state()
        data['oppo_sim_link_pQvw'] = self._sim_agent[opponent_idx].get_link_states()
        data['oppo_sim_joint_pv'] = self._sim_agent[opponent_idx].get_joint_states()
        data['oppo_sim_facing_frame'] = self._sim_agent[opponent_idx].get_facing_transform(self.get_ground_height(opponent_idx))
        data['oppo_sim_com'], data['oppo_sim_com_vel'] = self._sim_agent[opponent_idx].get_com_and_com_vel()
        
        data['oppo_kin_root_pQvw'] = self._kin_agent[opponent_idx].get_root_state()
        data['oppo_kin_link_pQvw'] = self._kin_agent[opponent_idx].get_link_states()
        data['oppo_kin_joint_pv'] = self._kin_agent[opponent_idx].get_joint_states()
        data['oppo_kin_facing_frame'] = self._kin_agent[opponent_idx].get_facing_transform(self.get_ground_height(opponent_idx))
        data['oppo_kin_com'], data['oppo_kin_com_vel'] = self._kin_agent[opponent_idx].get_com_and_com_vel()
    ...
    def get_task_error(self, idx, data_prev, data_next, actions):
        ...
        oppo_sim_root_p, oppo_sim_root_Q, oppo_sim_root_v, oppo_sim_root_w = data['oppo_sim_root_pQvw']
        oppo_sim_link_p, oppo_sim_link_Q, oppo_sim_link_v, oppo_sim_link_w = data['oppo_sim_link_pQvw']
        oppo_sim_joint_p, oppo_sim_joint_v = data['oppo_sim_joint_pv']
        oppo_sim_facing_frame = data['oppo_sim_facing_frame']
        oppo_R_sim_f, oppo_p_sim_f = conversions.T2Rp(oppo_sim_facing_frame)
        oppo_R_sim_f_inv = oppo_R_sim_f.transpose()
        oppo_sim_com, oppo_sim_com_vel = data['oppo_sim_com'], data['oppo_sim_com_vel']
        
        oppo_kin_root_p, oppo_kin_root_Q, oppo_kin_root_v, oppo_kin_root_w = data['oppo_kin_root_pQvw']
        oppo_kin_link_p, oppo_kin_link_Q, oppo_kin_link_v, oppo_kin_link_w = data['oppo_kin_link_pQvw']
        oppo_kin_joint_p, oppo_kin_joint_v = data['oppo_kin_joint_pv']
        oppo_kin_facing_frame = data['oppo_kin_facing_frame']
        oppo_R_kin_f, oppo_p_kin_f = conversions.T2Rp(oppo_kin_facing_frame)
        oppo_R_kin_f_inv = oppo_R_kin_f.transpose()
        oppo_kin_com, oppo_kin_com_vel = data['oppo_kin_com'], data['oppo_kin_com_vel']
        ...
        if self.exist_rew_fn_subterm(idx, 'oppo_ee'):
            error['oppo_ee'] = 0.0
            j = char_info.rwrist ## index of the right wrist
            sim_curr_ee_local = np.dot(R_sim_f_inv, sim_link_p[j]-p_sim_f)
            kin_curr_ee_local = np.dot(R_kin_f_inv, kin_link_p[j]-p_kin_f)
            
            sim_oppo_ee_local = np.dot(R_sim_f_inv, oppo_sim_link_p[j]-p_sim_f)
            kin_oppo_ee_local = np.dot(R_kin_f_inv, oppo_kin_link_p[j]-p_kin_f)    

            sim_ee_diff = sim_curr_ee_local - sim_oppo_ee_local
            kin_ee_diff = kin_curr_ee_local - kin_oppo_ee_local
            # print(self.get_current_time(),np.dot(kin_ee_diff,kin_ee_diff)**0.5)
            # print(self.get_current_time(),np.dot(kin_link_p[j]-oppo_kin_link_p[j],kin_link_p[j]-oppo_kin_link_p[j]))

            # sim_ee_diff = sim_link_p[j] - oppo_sim_link_p[j]
            # kin_ee_diff = kin_link_p[j] - oppo_kin_link_p[j]
            
            diff_pos = sim_ee_diff - kin_ee_diff
            error['oppo_ee'] = np.dot(diff_pos,diff_pos)
    ...
    def inspect_end_of_episode(self):
        ...
        for i in range(self._num_agent):
            ...
            if "interaction_fail":
                threshold = self._config['early_term'].get('interaction_thres')
                check = self.check_interaction_fail(i,threshold)
                if check: eoe_reason.append('[%s]:interaction is failed'%name)
        ...
        return eoe_reason        
    def check_interaction_fail(self,idx,thres):
        # oppo_idx = 1-idx
        # sim_agent = self._sim_agent[idx]
        # oppo_sim_agent = self._sim_agent[oppo_idx]

        # kin_agent = self._kin_agent[idx]
        # oppo_kin_agent = self._kin_agent[oppo_idx]
        # rwristIdx = sim_agent._char_info.rwrist


        # rwrist_kin_p,rwrist_kin_Q,rwrist_kin_v,rwrist_kin_w = kin_agent.get_link_states([rwristIdx])
        # rwrist_oppo_kin_p,rwrist_oppo_kin_Q,rwrist_oppo_kin_v,rwrist_oppo_kin_w = oppo_kin_agent.get_link_states([rwristIdx])
        # mocap_interaction_dist = np.dot(rwrist_kin_p[0]-rwrist_oppo_kin_p[0],rwrist_kin_p[0]-rwrist_oppo_kin_p[0])**0.5
        # if mocap_interaction_dist<thres:
        #     rwrist_sim_p,rwrist_sim_Q,rwrist_sim_v,rwrist_sim_w = sim_agent.get_link_states([rwristIdx])
        #     rwrist_oppo_sim_p,rwrist_oppo_sim_Q,rwrist_oppo_sim_v,rwrist_oppo_sim_w = oppo_sim_agent.get_link_states([rwristIdx])
        #     sim_interaction_dist = np.dot(rwrist_sim_p[0]-rwrist_oppo_sim_p[0],rwrist_sim_p[0]-rwrist_oppo_sim_p[0])**0.5
        #     if sim_interaction_dist<thres:
        #         return False
        #     else:
        #         print("Mocap hand interaction dist: %s. Sim hand interaction dist: %s"%(mocap_interaction_dist,sim_interaction_dist))
        #         return True
        # else:
        return False
    ...
    def render(self, rm):
        ...
        # rwristIdx = self._sim_agent[0]._char_info.rwrist
        # agent1 = self._sim_agent[0]
        # agent1_rwrist_p,agent1_rwrist_Q,agent1_rwrist_v,agent1_rwrist_w = agent1.get_link_states([rwristIdx])
        # rm.gl.glPushMatrix()
        # rm.gl.glTranslatef(agent1_rwrist_p[0][0], agent1_rwrist_p[0][1], agent1_rwrist_p[0][2])
        # rm.gl.glScalef(0.15,0.15,0.15)
        # rm.gl_render.render_sphere(
        #     constants.EYE_T, 0.4, color=[0, 1, 1, 1], slice1=16, slice2=16)
        # rm.gl.glPopMatrix()    
        # rm.gl.glPushMatrix()
        # agent2 = self._sim_agent[1]
        # agent2_rwrist_p,agent2_rwrist_Q,agent2_rwrist_v,agent2_rwrist_w = agent2.get_link_states([rwristIdx])
        # rm.gl.glTranslatef(agent2_rwrist_p[0][0], agent2_rwrist_p[0][1], agent2_rwrist_p[0][2])
        # rm.gl.glScalef(0.15,0.15,0.15)
        # rm.gl_render.render_sphere(
        #     constants.EYE_T, 0.4, color=[0, 1, 1, 1], slice1=16, slice2=16)
        # rm.gl.glPopMatrix()
