...
from envs import env_humanoid_imitation_retarget as my_env
...
class HumanoidImitationRetarget(gym.Env):
...
class EnvRenderer(er.EnvRenderer):
    ...
    def extra_keyboard_callback(self, key):
        if key == b'r':
            ...
            self.replay = False
            s = self.env.reset({'start_time': np.array([0])})
            self.reset()
        elif key == b'R':
...
env_cls = HumanoidImitationRetarget
