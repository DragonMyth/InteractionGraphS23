...
class Env(metaclass=ABCMeta):
    ...
    def __init__(self, config):
        ...
        for i in range(self._num_agent):
            if type(base_motion_file[i]) is list:
                m = asfamc.load(
                    base_motion_file[i][0],
                    motion=base_motion_file[i][1],
                    scale=1.0, 
                    load_skel=True,
                    load_motion=True,
            ...
            if reshape_ref:
                self._kin_agent.append(
                    sim_agent.SimAgent(pybullet_client=self._base_env._pb_client, 
                                    ...
                                    reshape_config = self._sim_agent[i]._reshape_config,
                                    kinematic_only=True,
    ...
    def get_current_pose_from_motion(self,idx):
        return self._ref_motion[idx].get_pose_by_time(self.get_ref_motion_time()[idx])
    ...
    def _state_object(self, 
        ...
        if type == "facing":
            T_ref=char_agent.get_facing_transform(self.get_ground_height(idx))
            include_com=True
            include_link_p=True
            include_link_Q=True
            include_link_v=True
            ...
            T_ref=char_agent.get_root_transform()
            include_com=False
            include_link_p=True
            include_link_Q=False
            include_link_v=True
    ...
    def _state_body(self, 
        ...
        if type == "facing":
            ...
            include_root_height=True
            include_root_up_dir=False
        elif type == "oppo_facing_R6_h":
            T_ref=oppo_agent.get_facing_transform(self.get_ground_height(1-idx))
            include_com=False
            include_link_p=True
            include_link_Q=False
            include_link_v=True
