...
from envs import env_humanoid_imitation_interaction as my_env
...
class HumanoidImitationInteraction(gym.Env):
    def __init__(self, env_config):
        ...
        self.dim_state_interaction_full = self.base_env.dim_interaction("full")
        self.dim_state_interaction_joint = self.base_env.dim_interaction("joint")
...
class EnvRenderer(er.EnvRenderer):
    ...
    def one_step(self, explore=None, collect_data=False):
        ...
        print("---Action Execution: %.4f seconds ---" % (time.time() - start_time))

        self.data['reward'].append(rew)
        self.data['time'].append(self.get_elapsed_time())
        self.data['states'].append((s1,s2))
    ...
    def extra_render_callback(self):
        ...
        if self.rm.get_flag('custom4'):
            ...
            inter_mesh = self.env.base_env.compute_interaction_mesh(self.env.base_env._sim_agent[0],self.env.base_env._sim_agent[0])
            
            axe[0,0].imshow(inter_mesh[:,:,0].transpose(1,0))
            axe[0,1].imshow(inter_mesh[:,:,1].transpose(1,0))
            axe[0,2].imshow(inter_mesh[:,:,2].transpose(1,0))

            axe[1,0].imshow(inter_mesh[:,:,3].transpose(1,0))
            axe[1,1].imshow(inter_mesh[:,:,4].transpose(1,0))
            axe[1,2].imshow(inter_mesh[:,:,5].transpose(1,0))
            plt.show()
...
env_cls = HumanoidImitationInteraction
...
def config_override(spec):
    ...
    if model and model == "interaction_net":
        ...
        if net_type=="conv":
            ...
            model_config.get("custom_model_config").update({
                "interaction_obs_dim": env.dim_state_interaction_joint,
                "interaction_obs_num": env.num_state_interaction,
            })
    del env
