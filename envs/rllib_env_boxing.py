...
from envs import env_humanoid_boxing as my_env
...
class HumanoidBoxing(MultiAgentEnv):
...
class MyCallbacksWithSchedule(MyCallbacksBase):
    ...
    def on_train_result(self, trainer, result: dict, **kwargs):
        def set_learnable(ev, policy_name, task_encoder, body_encoder, motor_decoder):
            model = ev.get_policy(policy_mapping_fn(policy_name, 0, self.share_policy)).model
